...
We conclude from this project that our controllers work good enough to control the rotary flexible joint in reality. But we have also noticed that the our controllers are not the same in simulation as in a lab. If you need to design a controller for real applications, you have to take into account so much more things such as saturation, quantization, noise, ... . We also realized that we can use a continuous model even when in reality it will be executed in a discrete way. As long as the sampling time is small enough it wont cause any problems.
